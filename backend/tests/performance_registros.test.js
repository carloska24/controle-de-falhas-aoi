...
describe('Performance - /api/registros', () => {
  let token;
  beforeAll(async () => {
    await initApp();
    ...
    // Se não veio no body, tenta extrair do set-cookie
    if (!token && loginRes.header['set-cookie']) {
      // Extrai token do cookie se necessário
  ...
  it('deve responder rápido para paginação padrão', async () => {
    ...
    const res = await request(app)
      .get('/api/registros?page=1&limit=50')
