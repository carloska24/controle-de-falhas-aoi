...
async function initializeDatabase() {
  ...
  // Migração: adicionar coluna prioridade se não existir
  try {
    await run(db, `ALTER TABLE registros ADD COLUMN prioridade TEXT;`);
  } catch (e) {
  ...
  await run(
    db,
    `CREATE TABLE IF NOT EXISTS oms_finalizadas (
  ...
  await run(
    db,
    `CREATE TABLE IF NOT EXISTS oms_pausadas (
      omNumber TEXT PRIMARY KEY,
      startTime INTEGER,
      pausedTime INTEGER,
      pauseStartedAt INTEGER,
      elapsedAtPause INTEGER,
      qtdlote INTEGER
    );`
  );
  ...
  await run(
    db,
    `CREATE INDEX IF NOT EXISTS idx_registros_createdat ON registros (createdat DESC);`
  );
  ...
  // Seed: garante que o DevAdmin sempre existe com a senha padrão em desenvolvimento
  const devAdmin = await get(db, 'SELECT id FROM users WHERE username = ?', ['DevAdmin']);
  if (!devAdmin) {
    ...
    await run(db, 'INSERT INTO users (name, username, password_hash, role) VALUES (?, ?, ?, ?)', [
      name,
      username,
      password_hash,
      role,
