...
# 3) tenta login com DevAdmin (credenciais padrão em dev)
Try {
    $body = @{ username = 'DevAdmin'; password = '123456' } | ConvertTo-Json
    $login = Invoke-RestMethod -Uri http://localhost:3001/api/auth/login -Method Post -ContentType 'application/json' -Body $body -TimeoutSec 8
...
# 4) se obteve token, chama endpoint para exportar sqlite (admin-only)
if (Test-Path login.json) {
    $obj = Get-Content login.json | ConvertFrom-Json
    if ($obj.token) {
        $token = $obj.token
        $out = Join-Path $backend ("aoi-download-" + (Get-Date -Format yyyyMMddHHmmss) + ".db")
        Try {
            Invoke-RestMethod -Uri http://localhost:3001/api/admin/export-sqlite -Headers @{ Authorization = "Bearer $token" } -OutFile $out -TimeoutSec 120
            Write-Output ("EXPORT_DONE: " + $out)
        } Catch {
            Write-Output "EXPORT_ERR: $($_.Exception.Message)"
        }
    } else {
        Write-Output "NO_TOKEN_IN_LOGIN"
    }
} else {
