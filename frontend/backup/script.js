document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function restaurarOM() {
    ...
    try {
      ...
      if (registros && registros.length > 0) {
        ...
        const regResp = await fetchAutenticado(`${API_URL}`);
  ...
  function resetForm() {
    ...
    // Só restaura se os campos não estiverem travados
    if (omInput && !omInput.readOnly) omInput.value = om; 
    if (qtdLoteInput && !qtdLoteInput.readOnly) qtdLoteInput.value = qtdlote;
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (editingId) {
      ...
      resetForm();
      await carregarRegistros(); // Recarrega para garantir consistência
    }
    } catch (error) {
