document.addEventListener('DOMContentLoaded', async () => {
  ...
  const btnNovaInspecao = document.querySelector('[data-action="new-om"]') || document.getElementById('btnNovaInspecao'); // <-- ADICIONADO
  ...
  async function startOM() {
    ...
    // 1. Verifica se a OM já existe no servidor
    ...
    if (existingOMData && existingOMData.status === 'pausada') {
        ...
        try {
            ...
            // Atualiza a UI para o estado "rodando"
            ensureOMDisplay();
            ...
            btnNovaInspecao.style.display = 'none'; // Esconde o botão "nova"
            if (btnPausarOM) {
            ...
            updateOMTimer(); // Atualiza imediatamente
            showToast(`OM ${omValue} retomada.`, 'info');

            // ================== NOVO TRECHO ADICIONADO ==================
            // Coloca a OM no campo de busca e filtra a tabela
            if (busca) {
                busca.value = omValue;
            }
            render(); // Atualiza a tabela para mostrar apenas os registros da OM retomada
    ...
    // 3. Se a OM não estiver pausada (nova, finalizada, ou em_andamento),
    //    deixa o endpoint /api/om/start (POST) lidar com isso.
    //    Ele já trata o caso de 'em_andamento' (erro) e 'finalizada' (inicia nova).
    ...
    try {
        ...
        if (!resp.ok) {
            ...
            throw new Error(errData.error || `Erro ${resp.status} ao iniciar OM`);
        }
        const data = await resp.json();
        
        // Configura o estado do cliente para a nova OM
        ...
        // Se falhar (ex: OM já em andamento), limpa o localStorage
  ...
  try { if (window.lucide && typeof lucide.createIcons === 'function') lucide.createIcons(); } catch (e) {}
    ...
    btnNovaInspecao.style.display = ''; // <-- ADICIONADO
    if (omTimer) clearInterval(omTimer);
  ...
  try { if (window.lucide && typeof lucide.createIcons === 'function') lucide.createIcons(); } catch (e) {}
    ...
    btnNovaInspecao.style.display = 'none'; // <-- ADICIONADO
    omTimer = setInterval(updateOMTimer, 1000);
    const omValue = localStorage.getItem('omEmAndamento') || document.getElementById('om').value;
    if (omValue) {
      await fetch(`/api/om/resume`, {
  ...
  if (btnNovaInspecao) btnNovaInspecao.addEventListener('click', () => {
    console.log('[OM] Clique em Nova Inspeção');
    resetParaNovaOM();
  });
  // ================== FIM DO LISTENER ==================
